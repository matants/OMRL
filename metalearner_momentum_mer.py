...
class MetaLearnerMomentumMer:
    ...
    def __init__(self, args):
        ...
        self.momentum = self.args.initial_momentum
        self.env.unwrapped.set_momentum(self.momentum)
        ...
        self.current_metatask_storage = None
    ...
    def train(self):
        ...
        for iter_ in range(self.args.num_iters):
            ...
            if iter_ == 0:
                self.initialize_current_metatask_storage()
                tasks_already_explored = set()
            ...
            if iter_ != 0 and iter_ % self.args.num_iters_per_momentum_update == 0:
                self.momentum += self.args.momentum_update_size
                self.env.unwrapped.set_momentum(self.momentum)
                self.initialize_current_metatask_storage()
                tasks_already_explored = set()
            ...
            for i in range(self.args.num_tasks_sample):
                ...
                tasks_already_explored.add(task)
            # update
            indices = np.random.choice(self.train_tasks, self.args.meta_batch)
            indices_from_already_explored = np.random.choice(list(tasks_already_explored), self.args.meta_batch)
            train_stats = self.update(indices, indices_from_already_explored)
    ...
    def update(self, tasks, tasks_already_explored_in_current_meta_task):
        '''
        Meta-update
        ...
        if self.args.log_tensorboard:
            ...
            if self.args.policy == 'dqn':
                ...
                for i, task in enumerate(self.train_tasks[:5]):
                    ...
                    self.tb_logger.writer.add_figure('rewards_pred_task_{}/halfway'.format(i),
                                                     utl_eval.vis_rew_pred(self.args, reward_preds[
                                                         i, int(np.ceil(reward_preds.shape[1] / 2))].round(2),
                                                                           self.env.goals[task]),
                                                     self._n_env_steps_total)
            ...
            if self.args.max_rollouts_per_task > 1:
                ...
                self.tb_logger.writer.add_scalar('returns/success_rate_train', np.mean(success_rate_train),
                                                 self._n_env_steps_total)
            # encoder
            self.tb_logger.writer.add_scalar('encoder/task_embedding_init', task_samples[:, 0].mean(),
                                             self._n_env_steps_total)
            self.tb_logger.writer.add_scalar('encoder/task_mu_init', task_means[:, 0].mean(), self._n_env_steps_total)
            self.tb_logger.writer.add_scalar('encoder/task_logvar_init', task_logvars[:, 0].mean(),
                                             self._n_env_steps_total)
            self.tb_logger.writer.add_scalar('encoder/task_embedding_halfway',
                                             task_samples[:, int(task_samples.shape[-1] / 2)].mean(),
                                             self._n_env_steps_total)
            self.tb_logger.writer.add_scalar('encoder/task_mu_halfway',
                                             task_means[:, int(task_means.shape[-1] / 2)].mean(),
                                             self._n_env_steps_total)
            self.tb_logger.writer.add_scalar('encoder/task_logvar_halfway',
                                             task_logvars[:, int(task_logvars.shape[-1] / 2)].mean(),
                                             self._n_env_steps_total)
            self.tb_logger.writer.add_scalar('encoder/task_embedding_final', task_samples[:, -1].mean(),
                                             self._n_env_steps_total)
            self.tb_logger.writer.add_scalar('encoder/task_mu_final', task_means[:, -1].mean(), self._n_env_steps_total)
            self.tb_logger.writer.add_scalar('encoder/task_logvar_final', task_logvars[:, -1].mean(),
                                             self._n_env_steps_total)
        ...
        for rollout in range(num_rollouts):
            ...
            while not done_rollout:
                ...
                _, task_mean, task_logvar, hidden_state = self.update_encoding(obs=next_obs,
                                                                               ...
                                                                               hidden_state=hidden_state)
                if self.args.fixed_latent_params:
                    task_mean = ptu.FloatTensor(utl.vertices(self.args.task_embedding_size)[self.task_idx]).reshape(
                        task_mean.shape)
                    task_logvar = -2. * ptu.ones_like(task_logvar)  # arbitrary negative enough number
    ...
    def sample_rl_batch(self, tasks, batch_size, is_sample_from_current_task=False):
        ''' sample batch of unordered rl training data from a list/array of tasks '''
        # this batch consists of transitions sampled randomly from replay buffer
        if is_sample_from_current_task:
            batches = [ptu.np_to_pytorch_batch(
                self.current_metatask_storage.random_batch(task, batch_size)) for task in tasks]
        else:
            batches = [ptu.np_to_pytorch_batch(
